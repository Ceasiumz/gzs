...
public class Matrix {
    ...
    public Matrix(int rows, int columns) {//int[rows][columns]
            ...
            for (int[] rowa : a.matrix) {//for each row in a
                for (int bcolcounter = 0; bcolcounter < b.columns; bcolcounter++) {//for each column in b
                    for (int i = 0; i < b.rows; i++) {
                        try {
                            rowab[i] = rowa[i] * b.matrix[i][bcolcounter];
                        } catch (Exception e) {
                            break;
                        }
                    }
    ...
    public static void main(String[] args) {
        ...
        try (Scanner input = new Scanner(System.in)) {
            ...
            c = new Matrix(para[0], para[1]);
            a = new Matrix(MatrixInput(input, para[0], para[1]));
            b = new Matrix(MatrixInput(input, para[1], para[2]));
